...
function wasEliminatedBeforePhase(teamNorm, faseGrupo, partidos) {
  const phasesBeforeTarget = {
    octavos: ["dieciseisavos"],
    cuartos: ["dieciseisavos", "octavos"],
    semifinales: ["dieciseisavos", "octavos", "cuartos"],
    final: ["dieciseisavos", "octavos", "cuartos", "semifinales"],
  };

  const phasesToCheck = phasesBeforeTarget[faseGrupo];
  if (!phasesToCheck) {
    return false;
  }

  for (const phase of phasesToCheck) {
    const result = getKnockoutMatchStatus(teamNorm, partidos, phase);
    ...
    if (isKnockoutPhaseComplete(phase, partidos)) {
      return true;
    }
...
function getTeamProgressStatus(teamNorm, faseGrupo, oficial) {
  ...
  if (wasEliminatedBeforePhase(teamNorm, faseGrupo, partidos)) {
    return "miss";
  }
