...
function resolveMatchScorePair(match) {
  ...
  return (
    readScorePair(score.regularTime) ||
    readScorePair(score.fullTime) ||
    readScorePair(score.extraTime) ||
    null
  );
}

function getMatchScore(match) {
  const pair = resolveMatchScorePair(match);
  if (!pair) {
    return null;
  }
...
function getWinnerFromApi(match, lookup) {
...
function getWinnerTeam(match, lookup) {
  const apiWinner = getWinnerFromApi(match, lookup);
  if (apiWinner) {
    return apiWinner;
  }
...
function matchToPartido(match, lookup) {
  ...
  return {
    ...
    marcadorPenales: finished ? getPenaltyScore(match) : null,
    ganador: finished ? getWinnerTeam(match, lookup) : null,
    definicion: finished ? getMatchDuration(match) : null,
    pendiente: !finished,
...
function computeGroupStandingsFromMatches(matches, lookup) {
  ...
  for (const match of matches) {
    ...
    const score = resolveMatchScorePair(match);
    if (!score) {
      continue;
    }
...
function enrichKnockoutWinners(partidos, resultadosFinales = {}) {
  const phaseFlow = [
    { from: "dieciseisavos", to: "octavos" },
    { from: "octavos", to: "cuartos" },
    { from: "cuartos", to: "semifinales" },
  ];

  function teamNorm(name) {
    return normalizeKey(name);
  }

  function teamsInPhase(faseGrupo, faseFilter = null) {
    const teams = new Set();
    ...
    for (const partido of partidos) {
      ...
      if (faseFilter && partido.fase !== faseFilter) {
        continue;
      }

      if (partido.equipo1) {
        teams.add(teamNorm(partido.equipo1));
      }

      if (partido.equipo2) {
        teams.add(teamNorm(partido.equipo2));
      }
    }
  ...
  function assignWinnerFromSets(partido, winnerSet, loserSet) {
    const t1 = teamNorm(partido.equipo1);
    const t2 = teamNorm(partido.equipo2);
    const t1Wins = winnerSet.has(t1);
    const t2Wins = winnerSet.has(t2);
    const t1Loses = loserSet.has(t1);
    const t2Loses = loserSet.has(t2);
  ...
  function maybeAssignTieWinner(partido, winnerForT1, winnerForT2) {
    if (partido.pendiente || partido.ganador || !partido.marcador) {
      return;
    }
    ...
    if (winnerForT1 && !winnerForT2) {
      partido.ganador = partido.equipo1;
    } else if (winnerForT2 && !winnerForT1) {
      partido.ganador = partido.equipo2;
    }
  }

  for (const { from, to } of phaseFlow) {
    const advancers = teamsInPhase(to);

    for (const partido of partidos) {
      maybeAssignTieWinner(
        partido,
        partido.faseGrupo === from && advancers.has(teamNorm(partido.equipo1)),
        partido.faseGrupo === from && advancers.has(teamNorm(partido.equipo2))
      );
    }
  }

  const finalTeams = teamsInPhase("final", "FINAL");
  const thirdTeams = teamsInPhase("final", "TERCER PUESTO");

  for (const partido of partidos) {
    if (partido.faseGrupo !== "semifinales") {
      continue;
    }

    maybeAssignTieWinner(
      partido,
      finalTeams.has(teamNorm(partido.equipo1)),
      finalTeams.has(teamNorm(partido.equipo2))
    );

    if (!partido.ganador) {
      assignWinnerFromSets(partido, finalTeams, thirdTeams);
    }
  }
  ...
  for (const partido of partidos) {
    ...
    if (partido.fase === "FINAL" && resultadosFinales.campeon) {
      const champ = teamNorm(resultadosFinales.campeon);
      if (champ === teamNorm(partido.equipo1)) {
        partido.ganador = partido.equipo1;
      } else if (champ === teamNorm(partido.equipo2)) {
        partido.ganador = partido.equipo2;
      }
      continue;
    }

    if (partido.fase === "TERCER PUESTO" && resultadosFinales.tercerPuesto) {
      const third = teamNorm(resultadosFinales.tercerPuesto);
      if (third === teamNorm(partido.equipo1)) {
        partido.ganador = partido.equipo1;
      } else if (third === teamNorm(partido.equipo2)) {
        partido.ganador = partido.equipo2;
      }
    }
  }
...
function transformFootballDataResponse({ matches, scorers, standings, aliases }) {
  ...
  enrichKnockoutWinners(partidos, resultadosFinales);
